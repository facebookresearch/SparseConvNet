...
at::Tensor pointToTensor(Point<Dimension> &p) {
    auto output = torch::empty({Dimension});
    ...
    for (Int i = 0; i < batchSize; i++) {
        ...
        for (auto const &localIter : local_SGs[i].mp) {
            ...
            if (globalIter != global_SGs[i].mp.end()) {
                ...
                // x is not in global
                if (global_input_features.device().is_cuda()) {
                    auto tmp = torch::zeros({global_input_features.size(1)}).cuda();
                    // tmp.zero_();
                    output_features.select(0, local_x_idx) = torch::cat({
...
/*
 * The local metadata == output metadata
 */
template <typename T, Int Dimension>
double cpu_GlobalFusion_backward(
        ...
        /*long*/ at::Tensor localBase, at::Tensor globalBase,
        Metadata<Dimension> &local, Metadata<Dimension> &global,
        /*float*/ at::Tensor local_input_grad, at::Tensor global_input_grad, at::Tensor output_grad,
        /*float*/ at::Tensor scale_ratio) {

    // Initialize Gradients
    local_input_grad.zero_();
    global_input_grad.zero_();
    auto local_feature_size = local_input_grad.size(1);
    auto global_feature_size = global_input_grad.size(1);
    
    auto local_iS = LongTensorToPoint<Dimension>(localInputSize);
    // auto output_iS = LongTensorToPoint<Dimension>(outputSize);
    auto global_iS = LongTensorToPoint<Dimension>(globalInputSize);
    ...
    for (Int i = 0; i < batchSize; i++) {
        ...
        for (auto const &localIter : local_SGs[i].mp) {
            ...
            local_input_grad.select(0, local_x_idx) = \
                output_grad.select(0, local_x_idx).slice(0, 0, local_feature_size);
            ...
            if (globalIter != global_SGs[i].mp.end()) {
                // x is in global
                auto global_x_idx = globalIter->second;
                global_input_grad.select(0, global_x_idx) = \
                    output_grad.select(0, local_x_idx).slice(0, local_feature_size,\
                            local_feature_size + global_feature_size);
            }
        }
    }
    return 0;
