      ...
      if (nHot > o)                                                            \
        dDeconvolution_KMxKN_forwardB<                                         \
            T, K, V><<<dim3(1, output_nPlanes / K, nGroups), dim3(K, K / V),   \
                       0, stream>>>(inFeatures, outFeatures, w, rules + 2 * o, \
                                    nHot - o, input_nPlanes, input_stride,     \
                                    output_nPlanes, output_stride);            \
      return;                                                                  \
    }                                                                          \
  }
    ...
    if (input_nPlanes % K == 0 and output_nPlanes % K == 0) {                  \
      ...
      if (nHot > o)                                                            \
        dDeconvolution_KMxKN_backward_dW_B<T, K, V><<<                         \
            dim3(1, input_nPlanes / K, nGroups), dim3(K, K / V), 0, stream>>>( \
...
double dDeconvolution_forward2(T *inFeatures, T *outFeatures, T *w,
  ...
  if (input_nPlanes % 8 != 0 or output_nPlanes % 8 != 0) {
    ...
    auto application = [&](Int *rbB, Int nHotB, cudaStream_t &stream) -> void {
      dDeconvolution_forward(inFeatures, outFeatures, w, rbB, nHotB,
